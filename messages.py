# ==================== BADİNİ KÜRTÇESİ MESAJLAR ====================

class Messages:
    # Temel Mesajlar
    BOT_NAME = "Botê Analîzê yê Grupê"
    WELCOME = "خێرهاتی"
    
    # Rapor Başlıkları
    REPORT_24H = "📊 داتایێن 24 سعەتا"
    REPORT_WEEKLY = "📈 داتایێن حەڤتیێ"
    ACTIVE_USERS = "👥 ئەندامێن اکتیڤ"
    INACTIVE_USERS = "💤 ئەندامێن نە اکتیڤ"
    TOP_MESSAGERS = "🏆 لیستا نامەیان"
    
    # Eksi Sistemi
    PENALTY = "⚠️ جزا"
    PENALTY_3 = "ئەو کەسێت گەهشتین 3 جزایا"
    WARNING = "وژداری"
    
    # İstatistik
    MESSAGE = "نامە"
    MEMBER = "ئەندام"
    TOTAL = "کۆ"
    
    # Komutlar
    REPORT = "📋 راپور"
    RELOAD = "🔄 بارکرن دوبارە"
    
    # Bildirimler
    SUCCESS = "✅ ب سەرکەڤتیانە"
    ERROR = "❌ خەلەتیەک چێبی"
    
    # Özel İfadeler
    NO_MESSAGES = "هیچ نامە رێنەکرە"
    MOST_ACTIVE = "ئەندامێ ژ هەمیان اکتیڤتر"
    INACTIVE_24H = "👤 د ماویێ 24 سعەتان دا ئەو کەسێن نە اخفتین:"
    PENALTY_3_LIST = "⚠️ ئەو کەسێن گەهشتین 3 جزایا:"
    RESET_PENALTY = "🔄 ب هنارتنا نامەکێ بو گروبی جزایێن خو ژێببە"
    WEEKLY_TOP = "👑 ئەندامێ د حەڤتیێ دا ژ هەمیان اکتیڤتر:"
    MESSAGE_LIST = "📝 لیستا نامەیان:"
    
    # Admin Mesajları
    NOT_ADMIN = "⛔ تە دەستوری ئەمە نینە!"
    ADMIN_UPDATED = "✅ لیستا ئەدمینان هاتە تازەکرن!"
    ADMIN_UPDATING = "🔄 لیستا ئەدمینان تازە دبیت..."
    REPORT_PREPARING = "🚀 رەپورت هاتیە ئامادەکرن..."
    NEED_ADMIN = "⚠️ ئەز ئەدمین نیمە! تکایە ئەز بکە ئەدمین."
    
    # Özel mesajlar
    PRIVATE_CHAT_ERROR = "❌ ئەز تەنێ د گروپان دا کار دکەم!"
    WRONG_GROUP_ERROR = "❌ ئەز تەنێ بۆ گروپێ تایبەت کار دکەم!"
    
    # Gelişmiş Mesajlar
    RANKING = "رێزبەندی"
    TOP10 = "توپ 10"
    WEEKLY_RANKING = "رێزبەندیا حەڤتیانە"
    MONTHLY_RANKING = "رێزبەندیا هەیفانە"
    ACTIVE_HOURS = "دەمژمێرێن اکتیڤ"
    GROUP_QUALITY = "کوالێتیا گروپی"
    BADGE = "مەدالی"
    LEVEL = "لیفل"
    XP = "پوینتێن تجروبێ"
    BADGE_7DAYS = "٧ روژا سەر ێک نامە"
    BADGE_1000MSG = "١٠٠٠ نامە"
    BADGE_500MSG = "٥٠٠ نامە"
    MY_RANK = "رێزبەندیامن"
    MY_LEVEL = "مستەوایێ من"
    WEEKLY_CHAMP = "سەرکەڤتیێ حەڤتیێ"
    CONGRAT_CHAMP = "دەستخوش سەرکەڤتیێ مە"
    QUALITY_SCORE = "کوالێتیا گروپی"
    ACTIVE_MEMBER_RATIO = "رێژا ئەندامێن اکتیڤ"
    SCORE = "سکور"
    EXCELLENT = "زورباشە"
    GOOD = "باشە"
    MEDIUM = "نافەراست"
    BAD = "خراب"
    
    # Seviye Mesajları
    LEVEL_EMOJI_IDS = {
        '1-5': '5999246912174166759',
        '6-10': '5197350061012436657',
        '11-15': '5409152396074952733',
        '16-20': '5224531551819165415',
        '21-25': '5253855995083371455',
        '26-30': '5289930954410836372',
        '31-40': '5231144435655390683',
        '41-50': '5470088387048266598',
        '51+': '5258112758645282249'
    }
    
    LEVEL_UP = "{} دەستخوش بویە لیفل {} {}\n\n{}"
    
    # Hatırlatma Mesajları
    REMINDER_24H = "⏰ {}، تە 24 سعەتا نامە نەرێکرە!\nب رێکرنا نامەکێ جزایێن خو ژێببە !"
    REMINDER_3DAYS = "🚨 {}، تە 3 روژە نامە نەرێکرە!\nئەگەر ناما رێنەکی، دێ ژ گروپی هیە دەرێخستن!"
    
    # Pasif Listesi
    PASSIVE_LIST = "💤 ئەو کەسێن 24 سعەتا نامە نەرێکرین:"
    
    # ==================== TOP MENU MESAJLARI ====================
    TOP_MENU_TITLE = "📋 **مێنو**"
    TOP_MENU_OPTIONS = """
1. راپورا ادمینا
2. نیکرنا لیستا ادمینا
3. پترین نامە ( 24 سعەت )
4. رێزبەندیا حەڤتیانە
5. رێزبەندیا هەیفانە
6. دەمێن اکتیڤ
7. کوالێتیا گروپی
8. سەرکەڤتیێ حەڤتیێ
9. رێزبەندیا من
10. رێزبەندیا لیفلا
11. ئەو کەسێن 24 سعەتان نە اخڤتین
"""
    TOP_MENU_PROMPT = "👉 ژمارەکێ هەلبژێرە (1-11):"
    TOP_MENU_INVALID = "❌ هەلبژارتن نە درستە! دوبارە هەول بدە."
